...
export function renderCard(record,{query='',compact=false}={}){
  const tracking=getTracking(record),favorite=isFavorite(record),link=getLinkCheck(record);
  const score=record._search?.profileMatch??record.profileMatch??0;
  const reasons=record._search?.reasons||[];
  const gaps=record._search?.gaps||[];
  return `<article class="result-card ${compact?'compact':''}" data-key="${escapeHTML(`${record.dataset}:${record.id}`)}">
    <button class="favorite ${favorite?'active':''}" data-action="favorite" aria-label="Favorite">${favorite?'★':'☆'}</button>
    <div class="record-kicker"><span>${escapeHTML(record.recordType||record.type)}</span><span>${escapeHTML(record.dataset)}</span></div>
    <h3>${escapeHTML(record.title)}</h3>
    <p class="record-subtitle">${escapeHTML(record.subtitle||record.type||'')}</p>
    <div class="badges">
      <span class="badge match">${score}% ${t('profileMatch')}</span>
      <span class="badge ${badgeClass(record.availability)}">${escapeHTML(record.availability)}</span>
      <span class="badge trust-${record.trustScore}">${escapeHTML(record.trust)}</span>
      ${link?`<span class="badge ${badgeClass(link.state)}">${escapeHTML(link.state)}</span>`:''}
    </div>
    <div class="record-details">
      <p><b>${t('country')}:</b> ${escapeHTML(record.country||record.region||'—')}</p>
      <p><b>${t('type')}:</b> ${escapeHTML(record.type||'—')}</p>
      <p><b>${t('lastChecked')}:</b> ${formatDate(record.checked)}</p>
      <p><b>${t('source')}:</b> ${escapeHTML(record.source||'—')}</p>
    </div>
    ${reasons.length?`<div class="match-reasons">${reasons.map(x=>`<span>✓ ${escapeHTML(x)}</span>`).join('')}</div>`:''}
    ${gaps.length?`<div class="match-gaps">${gaps.map(x=>`<span>△ ${escapeHTML(x)}</span>`).join('')}</div>`:''}
    <select class="status-select" data-action="status" aria-label="${t('status')}">${statusesFor(record).map(x=>`<option ${tracking.status===x?'selected':''}>${escapeHTML(x)}</option>`).join('')}</select>
    <div class="card-actions">
      <a class="button primary" href="${safeUrl(record.url)}" target="_blank" rel="noopener" data-action="open">${t('openSource')} ↗</a>
      <button class="button secondary" data-action="details">${t('details')}</button>
      <button class="button secondary compare-button ${comparison.has(compareKey(record))?'active':''}" data-action="compare">${t('compare')}</button>
    </div>
  </article>`;
}
